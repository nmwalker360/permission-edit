We use postsql for both local and server database. In order to run the sql. Tutor are require
to:

  1. run "$bundle install" command in package_delivery folder
  2. run "$sudo service postgresql start"(Ubuntu) command to start postsql server
